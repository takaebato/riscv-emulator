...
- Next: tighten Zicsr (a real CSR table instead of the permissive [u64; 4096]: unimplemented CSRs raise illegal-instruction; rv64mi-p-csr as the grader) to finish phase 2, then retry a real riscv64gc Linux binary (musl hello world; F/D instructions may force a design decision). Phase 3 (privileged modes, traps, CLINT, UART) follows. Commit at each step boundary.
