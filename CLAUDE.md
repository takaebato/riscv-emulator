...
- Next: the 7 remaining rv64ui-p failures, all illegal-instruction on missing decodes: SLTI/SLTIU/XORI/ORI/ANDI (OP-IMM funct3 gaps), JALR (opcode 0x67), FENCE.I (Zifencei, no-op for this interpreter). Add each to decode/execute and extend the PASSING list until it is the whole directory. Commit at each step boundary.
