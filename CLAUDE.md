...
- A extension, first half: the 9 AMOs × W/D widths (opcode 0x2f, funct5 in the funct7 top bits, aq/rl hint bits accepted and dropped). execute funnels all 18 through two read-modify-write helpers (amo_w/amo_d) taking the combining closure; .W sign-extends the old value into rd like LW. All 18 rv64ua-p-amo* tests pass the harness and run in lockstep with Spike.
- Next: LR/SC (funct5 00010/00011, a reservation flag on the Cpu — single-hart semantics), widening the rv64ua sweep to the whole directory. Then C (which real riscv64gc Linux binaries need), plus the rest of Zicsr. Commit at each step boundary.
