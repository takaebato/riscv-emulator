...
- Next: phase 3. Opening grader: rv64mi-p-csr (mstatus reset value 0xa00000080, WARL fields). Then the MIE/MPIE/MPP shuffle in trap()/MRET, privilege modes M/S/U (unlocks difftest's trap-path comparison and cause 8 vs 11), CLINT timer interrupts, and the UART as the first MMIO device. Commit at each step boundary.
