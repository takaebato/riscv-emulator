...
- Next: the CLINT (first MMIO device: msip/mtimecmp/mtime at 0x200_0000, Bus match arms, mtime advancing per step, MTIP wired to mtime >= mtimecmp), then the UART at 0x1000_0000 (guest-driven output), then the phase 3 boss: an interrupt-driven guest program. S mode waits for the phase 4 doorstep. Commit at each step boundary.
