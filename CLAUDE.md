...
- C extension under way: the variable-length infrastructure is in. `Cpu::fetch_decode` reads a 16-bit parcel first (low bits 11 → read the full 32-bit word, else it is compressed), `execute(inst, len)` derives the straight-line pc advance and jump link addresses from the instruction's own length, and `decode_compressed(u16)` is the third decoder stage, expanding compressed parcels into the same Inst enum (execute never knows compression exists). Quadrants 0 and 2 are populated: C.ADDI4SPN (all-zero parcel = defined illegal), C.LW/C.LD/C.SW/C.SD through the primed x8-x15 fields, C.SLLI, C.LWSP/C.LDSP/C.SWSP/C.SDSP against sp, and the funct3=100 five-way split (C.JR/C.MV/C.EBREAK/C.JALR/C.ADD, with reserved rd=0 cases rejected). Seven scaled-unsigned immediate extractors mirror the per-format bit scatters; decode tests check hand-split binaries against binutils-assembled hex (decode_compressed_checked). Q1 has only C.ADDI so far.
- Next: the rest of quadrant 1 (C.ADDIW/C.LI/C.LUI/C.ADDI16SP, the funct3=100 arithmetic block with SRLI/SRAI/ANDI/SUB/XOR/OR/AND/SUBW/ADDW, C.J/C.BEQZ/C.BNEZ), then rv64uc-p-rvc, the full difftest sweep, and a compressed guest build (-march=rv64imac). FP-only compressed forms stay illegal until F/D. Commit at each step boundary.
