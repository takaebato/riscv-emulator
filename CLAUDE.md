...
- **Phase 1 in progress**: cargo skeleton, ELF loader (`loader.rs`), exceptions (`exception.rs`), Bus (`bus.rs`, 128 MiB DRAM @ 0x8000_0000), Cpu (`cpu.rs`), and the two-stage decoder (`inst.rs`, with a `Display` disassembler). Implemented so far: ADDI, AUIPC, JAL, the six branches (B-type), the six Zicsr instructions (real semantics, `csrs: [u64; 4096]` storage; static read-only-CSR write check in decode), OP-IMM shifts (SLLI/SRLI/SRAI, 6-bit shamt), and the OP-IMM-32 W-family (ADDIW/SLLIW/SRLIW/SRAIW). rv64ui-p-add now runs 71 instructions and stops at `mret` (0x30200073), the last step of the riscv-tests trap setup.
- Next: a minimal MRET (jump to mepc; full privilege-mode handling waits for phase 3), then keep growing RV64I toward the test body. Commit at each step boundary.
