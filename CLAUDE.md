...
- Next: either retry a real riscv64gc Linux binary (musl hello world; F/D instructions likely force a design fork: implement F/D vs no-FP build vs trap-and-ignore) as the phase 2 epilogue, or go straight to phase 3 (mstatus for real: reset values, WARL masks, MIE/MPIE/MPP shuffle in trap()/MRET, privilege modes M/S/U, then CLINT timer interrupts and the UART). Commit at each step boundary.
