...
- C extension under way: the variable-length infrastructure is in. `Cpu::fetch_decode` reads a 16-bit parcel first (low bits 11 → read the full 32-bit word, else it is compressed), `execute(inst, len)` derives the straight-line pc advance and jump link addresses from the instruction's own length, and `decode_compressed(u16)` is the third decoder stage, expanding compressed parcels into the same Inst enum (execute never knows compression exists). First inhabitant: C.ADDI/C.NOP (quadrant 1, CI immediate format). All suites still pass and hold 86/86 difftest lockstep after the fetch-path change.
- Next: populate decode_compressed quadrant by quadrant (Q0: ADDI4SPN + loads/stores; Q2: SP-relative + JR/MV/ADD/EBREAK/SLLI; Q1: the rest), then the rv64uc-p-rvc test and a full-sweep regression. FP-only compressed forms (C.FLD etc.) stay illegal until F/D. Commit at each step boundary.
