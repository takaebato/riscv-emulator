...
- Next (milestone 1 remainder or infrastructure, in whichever order suits): syscall emulation for statically linked Linux ELFs (ECALL with a7=93 exit / a7=64 write instead of trapping to mtvec — the "hello world" boss fight), or the Spike diff-test harness (task #10, `spike -l --log-commits` vs our trace, first-divergence reporting). Phase 2 (M/A/C + the rest of Zicsr) follows. Commit at each step boundary.
