...
- **Phase 2 in progress**: the M extension is complete. Multiplies (MUL/MULH/MULHSU/MULHU on OP funct7=0000001, MULW on OP-32) compute 128-bit products via i128/u128 and return one half per instruction. Divides (DIV/DIVU/REM/REMU funct3 4-7, + the four W variants) never trap: division by zero yields all-ones quotients / dividend remainders, signed overflow MIN/-1 yields MIN/0 (Rust's wrapping_div/rem plus one explicit zero-divisor branch); DIVUW/REMUW still sign-extend their unsigned 32-bit results. All 13 rv64um-p tests pass the harness sweep and run in lockstep with Spike.
- Side quest done: `guest/fizzbuzz.s` (RV64IM, REMU branch selection, DIVU/REMU right-to-left decimal conversion into a .bss buffer) runs on the Linux emulation; `tests/hello_world.rs` pins its output against a Rust oracle via the shared run_guest helper. Cargo.toml sets default-run since the difftest binary joined.
- The A extension is complete. The 9 AMOs × W/D widths (opcode 0x2f, funct5 in the funct7 top bits, aq/rl hint bits accepted and dropped) funnel through two read-modify-write helpers taking the combining closure; .W sign-extends the old value into rd like LW. LR/SC add `Cpu::reservation: Option<u64>`: LR reserves after its fault check, SC stores only on an address match and always consumes the reservation (Option::take). Spike fails SCs spuriously on its 5000-instruction scheduling boundaries (spec-legal); difftest follows by dropping our reservation whenever the log shows a failed SC. All 19 rv64ua-p tests pass the harness sweep and run in lockstep with Spike.
- C extension under way: the variable-length infrastructure is in. `Cpu::fetch_decode` reads a 16-bit parcel first (low bits 11 → read the full 32-bit word, else it is compressed), `execute(inst, len)` derives the straight-line pc advance and jump link addresses from the instruction's own length, and `decode_compressed(u16)` is the third decoder stage, expanding compressed parcels into the same Inst enum (execute never knows compression exists). The C extension is complete (all three quadrants; FP-only forms C.FLD/C.FSD/C.FLDSP/C.FSDSP stay illegal until D). Q0: C.ADDI4SPN (all-zero parcel = defined illegal) + C.LW/C.LD/C.SW/C.SD via the primed x8-x15 fields. Q1: C.ADDI/C.ADDIW/C.LI, the rd-split C.ADDI16SP/C.LUI row, the funct3=100 arithmetic block (SRLI/SRAI/ANDI + SUB/XOR/OR/AND/SUBW/ADDW), and C.J/C.BEQZ/C.BNEZ (CJ/CB scatters). Q2: C.SLLI, the SP-relative loads/stores, and the funct3=100 five-way C.JR/C.MV/C.EBREAK/C.JALR/C.ADD split. Ten immediate extractors mirror the per-format scatters; decode tests check hand-split binaries against binutils-assembled words. rv64uc-p-rvc passes; all 87 tests (ui+um+ua+uc) hold difftest lockstep; an auto-compressed fizzbuzz (-march=rv64imac, .text 215→173 bytes) runs correctly. RV64IMAC is done.
- **Phase 2 (milestone 2 core) done**: Zicsr is tightened. `csr_exists` in decode whitelists the machine-mode CSRs phase 3 will model (0x300-0x306, 0x340-0x344, 0xf11-0xf14, satp, PMP ranges); anything else — notably mnstatus, which the test startup probes on purpose — raises illegal-instruction, exactly like Spike. That forced the architectural unification: harness::run and main's trace loop now route every escaped exception through Cpu::trap and continue (hardware never stops; unhandled traps spin into StepLimit), which deepened the difftest match — the mnstatus probe now traps identically on both sides. 87/87 lockstep holds. rv64mi-p-csr still fails at its mstatus read-back (Spike resets mstatus to 0xa00000080 — UXL/SXL hardwired 64-bit + MPIE — where our plain storage reads 0): that is precisely the phase 3 mstatus work, left as its opening grader.
- Recon of the real riscv64gc musl boss (a 6.6 MB static Rust hello world, built with `rustc --target riscv64gc-unknown-linux-musl -C linker=rust-lld -C target-feature=+crt-static`): four walls identified. (1) It links at 0x10000, outside DRAM — load fails immediately (StoreAmoAccessFault(65536)); needs low-address memory or remapping. (2) `_start` reads argc/argv/auxv from sp — the Linux process-startup stack must be materialized. (3) 165 F/D instructions are linked in via std's formatting machinery — the F/D design fork is real. (4) More syscalls (brk/mmap/set_tid_address...) surely follow. Compressed instructions, at least, already work. Deferred as a post-phase-3 campaign.
- **Phase 3 opened: mstatus + M/U privilege modes are real** (grader rv64mi-p-csr: PASS, 5237 instructions in full lockstep). `Cpu::csr_write` gives mstatus WARL semantics (writable-field mask, UXL hardwired to 2, SD derived from FS/VS dirtiness), misa introduces the machine honestly (RV64 IMAC+U, hardwired), mepc clears bit 0. trap() pushes the mstatus stack (MPIE←MIE, MIE←0, MPP←interrupted mode, rise to M); MRET pops it (drop to MPP's mode, MIE←MPIE, MPIE←1, MPP←U, MPRV cleared when leaving M). `Cpu::privilege` (M/U enum) gates CSR access dynamically in fetch_decode — CSR address bits 9:8 are the permission — and MRET itself; ecall's cause names the calling mode (8 from U, 11 from M). difftest's "stop at the final ecall" restriction is gone: all 88 tests (ui/um/ua/uc + mi-csr) run in full-trace lockstep, trap handlers included, against `spike --isa=rv64imac_zicsr_zifencei --priv=mu`. S mode (and its CSRs, medeleg/mideleg, SXL) deliberately deferred: claiming a mode obliges its registers.
- Interrupt architecture is in (device-independent half): `Cpu::take_pending_interrupt`, called at the top of step(), arbitrates mie & mip under the enable rule (below M: always; in M: only if mstatus.MIE), takes the highest-priority source (external 11 > software 3 > timer 7), and enters the handler with mepc = the not-yet-run instruction, mcause bit 63 | code, and vectored mtvec dispatch (base + 4×cause for interrupts only; exceptions stay at base). The mstatus push is shared with exception traps (push_trap_mstatus). Unit tests pin preemption, MIE masking (pending is not lost), vectored splitting, and M-interrupts preempting U unconditionally. 88/88 lockstep unchanged (nothing raises mip yet).
- The CLINT is in — the first MMIO device (`Clint` in bus.rs at 0x200_0000, QEMU-virt offsets: msip 0x0 as a 1-bit doorbell, mtimecmp 0x4000, mtime 0xbff8). load/store_bytes branch to it before DRAM, serving partial-width accesses out of each register's little-endian bytes; unmapped offsets in the window still fault. Time is defined as one mtime tick per step: `Cpu::tick` advances the clock and mirrors the device's interrupt wires into mip (MTIP = mtime >= mtimecmp, MSIP = msip bit). Writing mtimecmp is both "alarm at time T" and the interrupt acknowledgment. Tests pin the MMIO mapping, a programmed one-shot alarm preempting at the right step, and the doorbell raise/ack cycle. 88/88 lockstep unchanged (mtime differs from Spike's clock but nothing in the suites reads it).
- Next: the UART at 0x1000_0000 (minimal NS16550: THR write → host stdout, LSR reads ready; guest-driven output without ecall), then the phase 3 boss: an interrupt-driven guest program (timer-ticking, UART-printing). S mode waits for the phase 4 doorstep. Commit at each step boundary.
