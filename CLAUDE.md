...
- Next: the A extension (LR/SC + the 9 AMOs, opcode 0x2f; single-hart semantics make LR/SC reservations simple), then C (which real riscv64gc Linux binaries need), plus the rest of Zicsr. Optional side quest afterwards: a FizzBuzz guest/.s using the fresh div/rem through the Linux write syscall. Commit at each step boundary.
